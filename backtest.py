...
def fetch_and_process_data(tickers: list, period_days: int, interval: str, index_ema_span: int, ema_short: int, ema_long: int, ema_very_long: int):
    """
...
# --- Execution ---
df = fetch_and_process_data(TICKERS, period_days, interval, index_ema_span, ema_short, ema_long, ema_very_long)
...
for c in cycles:
    ...
    # Determine the lookback range for the lead contributor check (e.g., 10 bars before signal)
...
if df['signal'].abs().sum() == 0:
    ...
    sig_df = df[df['signal'] != 0][['index_cum_smooth','EMA_short','EMA_long','EMA_very_long','conviction_score', 'lead_contributor', 'ema_14_divergence', 'btc_stack_desc', 'eth_stack_desc', 'RSI','volume', 'Volume_MA','signal_reason','signal']].copy()
